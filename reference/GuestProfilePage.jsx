/**
 * curriculum.js — Static Apostolic Pentecostal Doctrinal Curriculum
 *
 * Schema for all OG (One True God) and NB (New Birth) domains and standards.
 * Designed for mastery-based, Jeremiah-led classroom progression.
 * OG must be mastered before NB is unlocked.
 *
 * Expandable: Add more domains/standards following the same schema.
 * Import this file and merge with Supabase-loaded data as needed.
 *
 * Evidence-of-Learning levels (in order):
...
export const curriculum = {
  ...
  OG: {
    ...
    color: "#003DA5",
    domains: [
      // ─── OG.1 ─── Absolute Divine Identity and Scriptural Monotheism ────
      {
        domainCode: "OG.1",
      ...
      // ─── OG.2 ─── The Absolute Deity of Jesus Christ ───────────────────
      {
        domainCode: "OG.2",
      ...
      // ─── OG.3 ─── The Father, Son, and Holy Spirit Relationship ────────
      {
        domainCode: "OG.3",
        domainTitle: "The Father, Son, and Holy Spirit — One God in Three Manifestations",
        anchorScripture: {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // NB — THE NEW BIRTH
  // (Locked until all OG standards are mastered)
  // ─────────────────────────────────────────────────────────────────────────
  NB: {
    code: "NB",
    title: "The New Birth",
    ...
    domains: [
      ...
      // ─── NB.2 ─── Water Baptism in Jesus's Name ─────────────────────────
      {
        domainCode: "NB.2",
...
/**
 * Returns all standards from the curriculum as a flat array.
 * Each entry includes: code, title, domainCode, domainTitle, subjectCode, evidenceOfLearning, anchorScriptures.
 */
export function getAllStandardsFromCurriculum(curriculumData = curriculum) {
  const all = [];
...
/**
 * Returns the first unmastered standard in OG-first order.
 * @param {object} progress — { [stdCode]: 0|1|2|3|4 }
 */
export function getNextUnmastered(progress = {}, curriculumData = curriculum) {
  const all = getAllStandardsFromCurriculum(curriculumData);
  return all.find((s) => (progress[s.code] || 0) < 4) || null;
}

/**
 * Returns true when all OG standards are mastered.
 * @param {object} progress — { [stdCode]: 0|1|2|3|4 }
 */
export function isOGMastered(progress = {}, curriculumData = curriculum) {
  const ogSubject = curriculumData.OG;
  if (!ogSubject) return false;
  return (ogSubject.domains || []).every((domain) =>
    (domain.standards || []).every(
      (std) => (progress[std.code] || 0) >= 4
    )
  );
}
